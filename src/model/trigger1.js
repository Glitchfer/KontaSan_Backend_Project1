  ...
  getTrigger1: (calendar) => {
    switch (calendar) {
      ...
      case "day":
        return new Promise((resolve, reject) => {
          connection.query(
            `SELECT *, GROUP_CONCAT(CONCAT_WS(  orders.item_quantity, product.product_name), ' ', CONCAT_WS(product.product_name, orders.item_quantity), 'x') AS 'orders', GROUP_CONCAT(orders.total_price) AS 'price' FROM invoice JOIN orders ON invoice.invoice_id = orders.invoice_id JOIN product ON orders.product_id = product.product_id WHERE DATE(updated_at) = DATE(NOW())`,
            (error, result) => {
              !error ? resolve(result) : reject(new Error(error));
            }
          );
      ...
      case "month":
        return new Promise((resolve, reject) => {
          connection.query(
            `SELECT *, GROUP_CONCAT(CONCAT_WS(  orders.item_quantity, product.product_name), ' ', CONCAT_WS(product.product_name, orders.item_quantity), 'x') AS 'orders', GROUP_CONCAT(orders.total_price) AS 'price' FROM invoice JOIN orders ON invoice.invoice_id = orders.invoice_id JOIN product ON orders.product_id = product.product_id WHERE CONCAT(YEAR(updated_at),'/',MONTH(updated_at))=CONCAT(YEAR(NOW()),'/',MONTH(NOW()))`,
            (error, result) => {
              !error ? resolve(result) : reject(new Error(error));
            }
          );
