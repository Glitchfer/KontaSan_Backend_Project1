...
module.exports = {
  getProduct: (limit, offset) => {
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT product_id, product_name, product_price, product_status, category_name,
            category_id, product_created_at, product_updated_at, img FROM product INNER JOIN category USING (category_id)
            LIMIT ? OFFSET ?`,
        [limit, offset],
        (error, result) => {
  ...
  getProductByName: (name, limit, offset) => {
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT * FROM product WHERE product_name LIKE "%"?"%" LIMIT ? OFFSET ?`,
        [name, limit, offset],
        (error, result) => {
          !error ? resolve(result) : reject(new Error(error));
        }
  ...
  sortProductByCategory: (value) => {
    return new Promise((resolve, reject) => {
      connection.query(
        "SELECT * FROM product WHERE category_id = ? ORDER BY product_name ASC",
        value,
        (error, result) => {
          !error ? resolve(result) : reject(new Error(error));
        }
  ...
  sortProductByRecent: (value) => {
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT * FROM product ORDER BY product_updated_at DESC`,
        value,
        (error, result) => {
          !error ? resolve(result) : reject(new Error(error));
        }
  ...
  sortProductByNumTable: (value) => {
    switch (value) {
      ...
      case "4":
        return new Promise((resolve, reject) => {
          connection.query(
            `SELECT * FROM product ORDER BY product_status ASC`,
            value,
            (error, result) => {
              !error ? resolve(result) : reject(new Error(error));
            }
      ...
      case "5":
        return new Promise((resolve, reject) => {
          connection.query(
            `SELECT * FROM product ORDER BY product_created_at DESC`,
            value,
            (error, result) => {
              !error ? resolve(result) : reject(new Error(error));
            }
